...
import us.databank.ReadingConnection;

public class DeleteDisciplineAction implements ActionListener{
	JFrame frame;
	...
	public void actionPerformed(ActionEvent e) {
		...
		ShowDisciplinesAction show = new ShowDisciplinesAction();
	...
	private void createBody(JFrame show){
		...
		JLabel tittleLBL = new JLabel("Remover Disciplina");
		...
		confirm.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				try {
					...
					}else if((new ReadingConnection().search("SELECT * FROM disciplina WHERE id ="+searchFLD.getText()).next())==false){
						new JOptionPane().showMessageDialog(null, "JÁ NÃO HÁ NENHUMA DISCIPLINA COM ESSE ID");
					}else {
						
						new Connectivity().sqlExec("DELETE FROM disciplina WHERE id ="+searchFLD.getText());
						show.dispose();
	...
	private void search(String id) {
		...
		try {
			...
			if((new ReadingConnection().search("SELECT * FROM disciplina WHERE id ="+id).next())==false) {
				new JOptionPane().showMessageDialog(null, "DISCIPLINA REMOVIDO COM SUCESSO");
			}else {
