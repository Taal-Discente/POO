...
public class DeleteCourseAction implements ActionListener{
	JFrame frame;
	...
	public void actionPerformed(ActionEvent e) {
		...
		ShowCoursesAction show = new ShowCoursesAction();
	...
	private void createBody(JFrame show){
		...
		JLabel tittleLBL = new JLabel("Remover Curso");
		...
		confirm.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				try {
					...
					}else if((new ReadingConnection().search("SELECT * FROM curso WHERE id ="+searchFLD.getText()).next())==false){
						new JOptionPane().showMessageDialog(null, "JÁ NÃO HÁ NENHUM CURSO COM ESSE ID");
					}else {
						
						new Connectivity().sqlExec("DELETE FROM curso WHERE id ="+searchFLD.getText());
						show.dispose();
	...
	private void search(String matricula) {
		...
		try {
			...
			if((new ReadingConnection().search("SELECT * FROM curso WHERE id ="+matricula).next())==false) {
				new JOptionPane().showMessageDialog(null, "CURSO REMOVIDO COM SUCESSO");
			}else {
